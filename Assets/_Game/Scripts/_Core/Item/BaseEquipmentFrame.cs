...
public abstract class BaseEquipmentFrame {
    // Lưu trữ vật phẩm theo vị trí tương ứng (Weapon -> Item, Head -> Item...)
    protected Dictionary<EquipmentSlot, BaseItem> EquippedItems;

    public BaseEquipmentFrame() {
        EquippedItems = new Dictionary<EquipmentSlot, BaseItem>();
    }

    // Kiểm tra xem vị trí đó có trống và vật phẩm có đúng loại không
    public abstract bool TryEquip(BaseItem item);
    public abstract BaseItem Unequip(EquipmentSlot slot);
    
    // Tính tổng chỉ số từ tất cả trang bị đang mặc
    public abstract float GetTotalBonusStat(StatType statType);
